...
class RotaryEncoder:
    ...
    def encoder_handler(self, pin):
        ...
        if time.ticks_diff(current_time, self.last_interrupt_time) > 5:
            a_state = self.pin_a.value()  # Current state of phase A
            b_state = self.pin_b.value()  # Current state of phase B
            
            # Only process if phase A state has changed
            if a_state != self.last_a_state:
                if b_state != a_state:
                    self.counter += 1  # Clockwise rotation
                else:
                    self.counter -= 1  # Counter-clockwise rotation
            
            self.last_a_state = a_state  # Update stored state
...
if __name__ == "__main__":
    """
    Example demonstration of the rotary encoder and button management system.
    
    This section shows how to use the classes in a typical application.
    """
    
    # Example callback functions
    def on_volume_change(steps):
        """Callback for encoder rotation - adjust volume."""
        direction = "up" if steps > 0 else "down"
        print(f"Volume {direction} by {abs(steps)} steps")
    
    def on_encoder_press():
        """Callback for encoder button press - play/pause."""
        print("Play/Pause media")
    
    def on_user_button():
        """Callback for user button - previous track."""
        print("Previous track")
    
    def on_power_long_press():
        """Callback for power button long press - shutdown."""
        print("System shutdown initiated")
    
    def counterclock(steps): 
        print(f"Counter Clock Pressed {steps}")
    # Create advanced button manager
    manager = AdvancedButtonManager()
    
    # Register callback functions
    manager.set_encoder_callback('clockwise', on_volume_change)
    manager.set_encoder_callback('counterclockwise', counterclock)
    manager.set_button_callback('encoder', 'short', on_encoder_press)
    manager.set_button_callback('user', 'short', on_user_button)
    manager.set_button_callback('user', 'long', on_power_long_press)
    
    print("Rotary encoder system ready. Start turning encoder or pressing buttons...")
    print("Rotary encoder A: GPIO{}, B: GPIO{}, Button: GPIO{}".format(
        ENCODER_A_PIN, ENCODER_B_PIN, ENCODER_BUTTON_PIN))
    print("Buttons: User: GPIO{}, Power: GPIO{}".format(USER_BUTTON_PIN, PWR_EN_PIN))
    
    # Main application loop
    try:
        while True:
            manager.update()  # Process input events and trigger callbacks
